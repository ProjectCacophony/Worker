...
type bundle map[bundleInfo][]Entry

func (p *Plugin) bundleEntries(run *common.Run, tx *sql.Tx, entries []Entry) bundle {
	var err error
	...
	bundledEntries := make(map[bundleInfo][]Entry)
	...
	for _, entry := range entries {
		...
		for key := range bundledEntries {
			if key.ChannelID != entry.WeverseChannelID {
				continue
			}
		...
		if !addedToBundle {
			bundledEntries[bundleInfo{
				ChannelID: entry.WeverseChannelID,
			}] = []Entry{entry}
		}
	}
