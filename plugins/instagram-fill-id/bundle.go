...
type checkBundleInfo struct {
	Username string
}

type checkBundles map[checkBundleInfo][]Entry

func (p *Plugin) bundleEntries(run *common.Run, tx *sql.Tx, entries []Entry) checkBundles {
	var err error
	...
	bundledEntries := make(map[checkBundleInfo][]Entry)
	...
	for _, entry := range entries {
		...
		for key := range bundledEntries {
			if key.Username != entry.InstagramUsername {
				continue
			}
		...
		if !addedToBundle {
			bundledEntries[checkBundleInfo{
				Username: entry.InstagramUsername,
			}] = []Entry{entry}
		}
	}
