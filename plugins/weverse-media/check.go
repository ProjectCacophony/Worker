...
func (p *Plugin) checkBundles(run *common.Run, tx *sql.Tx, bundles bundle) {
	...
	var mediaFeed *geverse.MediaPosts
	for checkInfo, entries := range bundles {
		mediaFeed, err = p.geverse.GetMediaPosts(run.Context(), checkInfo.ChannelID)
		if err != nil {
		...
		for _, entry := range entries {
			err = p.checkEntry(run, entry, mediaFeed.Medias)
			if err != nil {
				run.Except(err, "channel_id", strconv.FormatInt(checkInfo.ChannelID, 10))
...
func (p *Plugin) checkEntry(run *common.Run, entry Entry, posts []geverse.MediaPost) error {
	var posted int
...
func (p *Plugin) post(_ *common.Run, entry Entry, post *geverse.MediaPost) error {
	var err error
	...
	if post.Video.ThumbnailImgPath != "" {
		// TODO: figure out how to handle videos
		mediaURLsRaw = append(mediaURLsRaw, post.Video.ThumbnailImgPath)
	}
	if post.ExtVideoPath != "" {
		mediaURLsRaw = append(mediaURLsRaw, post.ExtVideoPath)
	}
	mediaURLsRaw = append(mediaURLsRaw, extractMediaURLs(post.Photos)...)
	mediaURLs := splitURLs(mediaURLsRaw)
	...
	messages, err := discord.SendComplexWithVars(
		...
		&discordgo.MessageSend{
			Content: "weverse-media.post.content",
		},
